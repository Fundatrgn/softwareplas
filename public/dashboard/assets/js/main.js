$(function() {
  ...
  $(".dark-mode-icon").on("click", function() {
    ...
    if($(".mode-icon ion-icon").attr("name") == 'sunny-outline') {
        ...
        localStorage.setItem("adminTheme", "light-theme");
    } else {
        $(".mode-icon ion-icon").attr("name", "sunny-outline");
        $("html").attr("class", "dark-theme")
        localStorage.setItem("adminTheme", "dark-theme");
    }

  }),

  // Sayfa yüklendiğinde, kaydedilmiş tema tercihine göre hem üst
  // menüdeki ay/güneş ikonunu hem de "Theme Customizer" panelindeki
  // seçili radyo düğmesini doğru göster (html class'ı zaten <head>
  // içindeki erken script tarafından ayarlandı).
  (function () {
    var saved = localStorage.getItem("adminTheme");
    if (saved === "dark-theme") {
      $(".mode-icon ion-icon").attr("name", "sunny-outline");
      $("#DarkTheme").prop("checked", true);
    } else if (saved === "semi-dark") {
...
$("#SemiDark").on("click", function() {
  ...
  localStorage.setItem("adminTheme", "semi-dark");
}),
