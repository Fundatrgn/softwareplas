<div class="text-center p-3" style="color:#8a8a8a; font-size:13px;">
    © {{ date('Y') }} {{ $settings?->footer_copyright_text ?: 'Yunuscan ZEYBEK - Tüm Hakları Saklıdır' }}
</div>
...
<!-- Main JS-->
<script src="{{ asset('dashboard/assets/js/main.js') }}?v=2"></script>
<script>
    // Zengin metin (CKEditor) alanları: "js-editor" sınıfı ya da eski
    // formlardaki id="editor" olan her textarea editöre dönüştürülür.
    // CDN'e erişilemezse alanlar düz metin kutusu olarak çalışmaya devam eder.
    window.CKEDITOR && document.querySelectorAll('textarea.js-editor, textarea#editor').forEach(function (el) {
        if (el.disabled || (el.id && CKEDITOR.instances[el.id])) {
            return;
        }
        CKEDITOR.replace(el, {
